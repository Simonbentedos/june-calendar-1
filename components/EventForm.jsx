...
export default function EventForm({
	...
	const getInitialColor = () => {
		...
		if (typeof event.color === "string" && event.color.startsWith("#")) {
			...
			if (matchingColor) {
				...
				// It's a custom color - generate the complementary colors
				const colorHex = event.color.slice(1) // Remove #

				// Create lighter shade for background
				const r = parseInt(colorHex.substr(0, 2), 16)
				const g = parseInt(colorHex.substr(2, 2), 16)
				const b = parseInt(colorHex.substr(4, 2), 16)

				// Light version (very light tint)
				...
				const borderR = Math.max(0, r - 15).toString(16).padStart(2, "0")
				const borderG = Math.max(0, g - 15).toString(16).padStart(2, "0")
				const borderB = Math.max(0, b - 15).toString(16).padStart(2, "0")
				const borderColor = `#${borderR}${borderG}${borderB}`

				// Text color (much darker for contrast)
				const textR = Math.max(0, Math.floor(r * 0.6)).toString(16).padStart(2, "0")
				const textG = Math.max(0, Math.floor(g * 0.6)).toString(16).padStart(2, "0")
				const textB = Math.max(0, Math.floor(b * 0.6)).toString(16).padStart(2, "0")
				const textColor = `#${textR}${textG}${textB}`
	...
	const [formData, setFormData] = useState({
		...
		customColor:
			event?.color && !eventColors.find((c) => c.value === event.color)
				? event.color
				: "#3B82F6",
	})
	...
	const handleCustomColorChange = (e) => {
		...
		// Generate complementary colors for the custom color
		const colorHex = customColor.slice(1) // Remove #
